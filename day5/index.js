...
function setupCrates(stacksString) {
  const stackArr = stacksString.split("\n");
  const crates = []
  let positions = 0;
  stackArr.pop().split(' ').reduce((arr, position) => {
...
function setupDirections(directionString){
  const directionArr = directionString.split("\n")
  const directions = [];
  directionArr.forEach((direction) => {
    directions.push(direction.split(" ").reduce((arr, el) => {
       if(parseInt(el)){
          arr.push(el)
        }
        return arr
...
function run() {
  const [stacks, directionString] = fs.readFileSync('./input.txt', 'utf-8').split('\n\n');
